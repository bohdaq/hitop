...
function App() {
  ...
  // Tabs and current tab are in sessionStorage (sync). Everything else loads async.
  const getSyncInitialState = () => {
  ...
  const syncState = getSyncInitialState();
  const [tabs, setTabs] = useState(syncState.tabs);
  const [currentTab, setCurrentTab] = useState(syncState.currentTab);
  const [collections, setCollections] = useState(syncState.collections);
  const [requestHistory, setRequestHistory] = useState(syncState.history);
  ...
  // Load persisted data from Rust backend on first mount
  useEffect(() => {
    (async () => {
      const [savedCollections, savedHistory, savedContexts] = await Promise.all([
        storageService.loadCollections(),
        storageService.loadHistory(),
        storageService.loadContexts(),
      ]);
      if (savedCollections && savedCollections.length > 0) setCollections(savedCollections);
      if (savedHistory) setRequestHistory(savedHistory);
      if (savedContexts) setCollectionContexts(savedContexts);
    })();
